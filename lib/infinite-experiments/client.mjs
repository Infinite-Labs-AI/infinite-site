...
export function createExperimentClient(options = {}) {
  ...
  // The consent cookie records an explicit decision and fails closed: 'granted' only for an explicit grant, nothing under a
  // not_required policy without one, 'denied' otherwise. Only an explicit negative leaves no experiment identifier behind:
  // the person cookie and every assignment cookie expire (the middleware sets neither HttpOnly), so a later grant enrols as
  // a new person. An undecided site, an unreadable owner or a privacy signal keeps the identity for the decision to come.
  function bridge() {
    const state=decision();
    if(state==='granted') put(consent.name,consent.grantedValue);
    else if(state!=='not_required') put(consent.name,consent.deniedValue);
    if(state==='denied') {
      let names=[];
