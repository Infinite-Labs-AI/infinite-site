...
which runs `scripts/prepare-static-deploy.cjs`. That script creates the
published `dist/` directory from an explicit allow-list plus the current
homepage artifact. Anything not copied by the script — including this
...
- `2026-07-pre-wrangle-site/index.html` — the previous production root
  homepage, archived when the Wrangle-inspired static build became the deploy
  source for `infinite.site`.
