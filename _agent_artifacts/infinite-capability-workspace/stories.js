(() => {
 ...
 section.querySelectorAll('[data-story-open]').forEach(button=>button.addEventListener('click',()=>{opener=button;dialog.querySelector('.result-dialog-subtitle').textContent=(panels[Number(button.dataset.storyOpen)].querySelector('.result-quote')||panels[Number(button.dataset.storyOpen)].querySelector('h3'))?.textContent||'';dialog.showModal();}));
