...
| In-flight ad click identifiers: Meta `_fbc` / `_fbp` cookie values and a `gclid` present on the `/get-started` URL, carried on the claim request only when the consent gate has started analytics | Forwarded by the same server invocation to Meta / Google so the verified-sign-up conversion matches the ad click | Never at rest: not written to browser storage, not stored in `handoff_claims`, not logged, not queued; the cloud's `handoff_conversion_events` audit row holds platform, event, event id, status and error class only | Browser → claim route → Meta Conversions API / Google Data Manager API; the Meta pixel (when `INFINITE_META_PIXEL_ID` is configured) mirrors `CompleteRegistration` with `eventID = claimId` for dedup |
