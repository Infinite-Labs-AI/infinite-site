...
**Goal:** Add an always-visible, 21-row Max/Ultra feature comparison beneath the existing compact homepage pricing cards.
...
- Use the exact 21 bundled capabilities and plan assignments in `docs/superpowers/specs/2026-08-25-expanded-pricing-matrix-design.md`.
...
- Produces: source checks for `[data-pricing-proof]`, `[data-pricing-matrix]`, eight `[data-pricing-group]` elements, 21 `[data-pricing-feature]` rows, five `[data-ultra-only="true"]` rows, the six `Best for` statements, and `[data-pricing-closing-cta]`.

- [ ] Add a failing verifier block that asserts:
  - proof-strip text includes `79 agent tools` and `One growth operating system`;
  - group IDs are `operator`, `leads`, `seo`, `ads`, `content`, `conversion`, `analytics`, `brand`, and `competitive`;
  - feature-row count is 21;
...
- [ ] Add all eight group sections and all 21 bundled rows.
...
- [ ] Verify `dist/index.html` contains eight groups, 21 rows, five Ultra-only rows, six `Best for` statements, and no stale prices.
